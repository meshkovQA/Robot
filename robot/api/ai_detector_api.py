...
def add_ai_detector_routes(bp: Blueprint, *, ai_detector, camera, ai_runtime, ok, err):
    ...
    def ai_stream():
        # ВНИМАНИЕ: это выполняет детекцию на каждый кадр (дорого).
        # Оставим как «демо», но лучше рисовать по кэшированным детекциям из ai_runtime.
        def generate():
            while True:
                try:
                    if not camera:
                    ...
                    detections = ai_detector.detect_objects(frame)  # дорого
                    annotated_frame = ai_detector.draw_detections(
                        frame.copy(), detections)
