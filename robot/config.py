...
# --- Удержание курса (PID по углу Yaw) ---
HDG_HOLD_ENABLED = True
HDG_KP = 0.9                     # пропорциональный коэффициент
# интегральный коэффициент (обычно 0, чтобы не уплывал)
HDG_KI = 0.0
HDG_KD = 0.05                    # дифференциальный коэффициент
# "мёртвая зона": ошибка меньше этого игнорируется (°)
HDG_ERR_DEADZONE_DEG = 1.5
# максимальная длительность коррекционного импульса (мс)
HDG_MAX_CORR_PULSE_MS = 120
HDG_MIN_GAP_BETWEEN_PULSES_MS = 150  # минимальная пауза между импульсами (мс)

# скорость (PWM), с которой выполняются корректирующие повороты
HDG_CORR_SPEED = 120
